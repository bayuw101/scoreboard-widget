...
const CarouselHeader = ({
  ...
  const handleSportClick = (sport: sportsType) => {
    setSelectedSport(
      selectedSport?.sportId === sport.sportId ? null : sport
    );
  };
  ...
  // Render "More Sports" dropdown
  const renderMoreSportsDropdown = () => (
    <div className="relative z-10 more-sports">
      ...
      {showMore && (
        <div className="absolute text-black bg-white rounded-md shadow-md w-auto min-w-[12rem]">
          <input
            type="text"
            placeholder="Search..."
      ...
      >
        ...
        <div className="flex items-center justify-between">
          ...
          >
            ...
            <div className="border-l-0 border-[#595180] flex items-center h-12">
            <div className="flex space-x-4 border-[#3c365a] border-l-0 px-4 font-semibold items-center">
              <button onClick={() => setSelectedSport(null) } className="text-sm text-gray-100 h-[28px] hover:text-gray-300 cursor-pointer">
              All Sports
              </button>
              {renderSportsButtons()}
              {sports.length > (rootWidth <= 1025 ? 0 : 5) &&
              renderMoreSportsDropdown()}
            </div>
            </div>
        </div>
        ...
        <div className="p-4 relative select-date flex items-center justify-center gap-2">
          <span className="text-gray-100 text-xs font-semibold mt-[2px] hidden md:block mr-2">
