...
interface CanvasBlackImageProps {
  imageUrl: string;
...
const CanvasBlackImage: React.FC<CanvasBlackImageProps> = ({
  imageUrl,
  ...
  const processImage = (img: HTMLImageElement) => {
    ...
    for (let i = 0; i < data.length; i += 4) {
      ...
      if (isBackground) {
        ...
        // Recolor non-background pixels to black
        data[i] = 0; // R
        data[i + 1] = 0; // G
        data[i + 2] = 0; // B
      ...
      {processedImage ? (
        <img
          ...
          alt={
            error
              ? "Fallback content"
              : "White image with transparent background"
          }
          style={{ background: "transparent" }}
        />
      ) : (
        <div
          className={`image-loader rounded-full ${className} bg-gray-300 animate-pulse`}
        ></div>
      )}
    </div>
  );
...
export default CanvasBlackImage;
